...
st.set_page_config(page_title="ANALYSE DE DONNEES FISCALES", layout="wide")

# Titre de la page avec formatage HTML pour le style
st.sidebar.image("image/ml.png", caption="MULTI-VARIATION D'ANALYSE")
st.markdown("<h1 style='text-align: center; font-size: 40px;'>ANALYSE PREDICTIVE DES REDRESSEMENTS FISCAUX 💹</h1>", unsafe_allow_html=True)
...
if uploaded_file is not None:
    ...
    def calcul_indicateur_operation_locale(row):
        ecart = max(max(row['DCOM_recoup'], row.get('Annexes_TVA_recoupées', 0)) - row['CA_local'], 0)
        return ecart / row['CA_IR'] if row['CA_IR'] != 0 else 1
    ...
    try:
        ...
        # Suppression des lignes avec des valeurs manquantes dans X et y
        ...
        if X_cleaned.empty or y_cleaned.empty:
            ...
            data_for_line_chart['Index'] = data_for_line_chart.index
            ...
            line_chart = px.line(data_for_line_chart, x='Index',
                                ...
                                title='Comparaison des Indicateurs Fiscales',
...
if uploaded_new_file is not None:
    ...
    # Affichage des données du nouveau DataFrame
    ...
    # Afficher les données préparées pour la régression
    st.write("Préparation des données pour la régression :")
    st.dataframe(new_df[columns_required])  # Affichez les colonnes requises de votre nouveau DataFrame

    # Gestion des valeurs manquantes avant de prédire
    try:
        # Suppression de la colonne pour prédiction
        X_new = new_df.select_dtypes(include=[np.number])

        # Vérification que les colonnes d'entrées sont cohérentes avec ce qui a été utilisé pour entraîner le modèle
        X_cleaned_columns = X_cleaned.columns.tolist()  # Conserver les noms des colonnes d'origine
        X_new = X_new.reindex(columns=X_cleaned_columns, fill_value=0)

        # Afficher les données avant l'imputation
        st.write("Données avant imputation :")
        st.dataframe(X_new)

        # Imputer les valeurs manquantes
        if X_new.isnull().values.any():
            st.warning("Des valeurs manquantes ont été trouvées dans les données, elles seront imputées par la moyenne.")
